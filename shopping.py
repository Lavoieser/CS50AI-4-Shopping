...
def evaluate(labels, predictions):
    ...
    True_positive are the cases where predictions == 1 AND Labels == 1, Total = 2
    True_negative are the cases where predictions == 0 AND Labels == 0, Total = 0
    In the cases where predictions is False, the Total = 0 + 1 or 1 + 0 == 1
    """
    len_of_data = len(labels)
    total_positive = 0
    for row in labels:
        total_positive += row
...
def load_data(filename):
    ...
    with open(filename) as f:
        reader = csv.reader(f)
        next(reader)    # To step over the title line of the csv file
